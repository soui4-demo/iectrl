...
namespace SOUI
{
    ...
    void SIECtrl::OnAxActivate( IUnknown *pUnknwn )
    {
        ...
        if(m_pIE)
        {
			...
			SDocHostUIHandler *pDocHostUIHandler = new SDocHostUIHandler(this);
	...
	BOOL SIECtrl::FunctionNameToDispatchId(LPOLESTR szName, DISPID *pDispId){
		
		if(wcscmp(szName,L"CppCall")==0)
		{
			*pDispId = 1;
			return TRUE;
		}
	...
	void SIECtrl::CallCPlusPlus(DWORD dispatchId, const std::vector<std::wstring>& args, VARIANT* pResult){
		if(dispatchId == 1)
		{//CppCall
			SStringW strMsg=SStringW().Format(L"Call CppCall(str) from js, param1:%s",args[0].c_str());
			SMessageBox(GetContainer()->GetHostHwnd(),S_CW2T(strMsg),_T("haha"),MB_OK);
		}
	}
